...
import { Box, Stack, Container, ThemeProvider, CssBaseline, Typography } from '@mui/material/';
import { createTheme } from '@mui/material/styles';
import { useSelector } from "react-redux";

const darkTheme = createTheme({
  palette: {
    mode: "dark", background: { default: "#121212", paper: "#1E1E1E" },
    text: { primary: "#FFFFFF", secondary: "#B3B3B3" },
    primary: { main: "#90caf9" }, secondary: { main: "#f48fb1" },
  },
  typography: { h3: { color: "#FFFFFF" }, body1: { color: "#B3B3B3" } },
});
          ...
          <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
            {counts.map((count, index) => (
              <Box key={index} sx={{ flex: 1, backgroundColor: "background.paper", borderRadius: 2, padding: 2 }}>
                <CountsCard name={count.name} icon={count.icon} desc={count.desc} value={"100"} color={count.color} lightColor={count.lightColor} />
              </Box>
            ))}
          </Stack>
